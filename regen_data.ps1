...
for ($i = 1; $i -lt $lines.Length; $i++) {
    ...
    $storeName = if ($colLoja -ne -1 -and $null -ne $cols[$colLoja]) { $cols[$colLoja].Trim() } else { "" }
    $productName = if ($colItem -ne -1 -and $null -ne $cols[$colItem]) { $cols[$colItem].Trim() } else { "" }
    ...
    if ($storeName -ne "" -and (-not $storeMap.ContainsKey($storeName.ToLower()))) {
        $raw = "$networkName-$storeName"
        $idStr = $raw.Normalize("FormD") -replace "\p{M}", "" -replace "[^a-zA-Z0-9-]", "-" -replace "-+", "-" -replace "^-|-`$", ""
        $idStr = $idStr.ToLower()
        $storeObj = @{ id = $idStr; name = $storeName; network = $networkName; lastVisit = $null; status = 'pending'; productIds = @() }
        $stores += $storeObj
    ...
    if ($productName -ne "" -and (-not $productMap.ContainsKey($productName.ToLower()))) {
        ...
        $idStr = $raw.Normalize("FormD") -replace "\p{M}", "" -replace "[^a-zA-Z0-9-]", "-" -replace "-+", "-" -replace "^-|-`$", ""
        $idStr = $idStr.ToLower()
        $prodObj = @{ id = $idStr; name = $productName; network = $networkName; status = $status }
        $products += $prodObj
...
Write-Output "data.js foi regenerado com sucesso usando IDs fixos em texto!"
