...
for ($i = 1; $i -lt $lines.Length; $i++) {
    ...
    # Forcefully fix REFORÃ‡ADO regardless of the corrupted character
    if ($productName -match "REFOR.*ADO") {
        $productName = "REFORÃ‡ADO " + ($productName -replace ".*ADO\s*", "").Trim()
    }
...
Write-Output "Regenerated perfectly with explicit fix."
, ''; $id = $id.ToLower()
        $storeObj = @{ id = $id; name = $storeName; network = $networkName; lastVisit = $null; status = 'pending'; productIds = @() }
        $stores += $storeObj
        $storeMap[$storeName.ToLower()] = $storeObj
